import os
import sys
import subprocess
from google import genai
import json
from create_action.json_load import save_json_safely
import re
def clean_response(text):
    # Remove markdown code block markers
    text = re.sub(r"```(json)?", "", text).strip()

    # Extract first valid JSON object from the text
    match = re.search(r"\{[\s\S]*?\}", text)
    return match.group() if match else text
def file_ops(user_msg):
    system_prompt = '''
You are SoulOS's strict intent classifier.

Your job is to classify the `user_input` based only on **clearly stated file creation commands**.

🎯 Instructions:
- Match only if the `user_input` includes clear trigger words like:
  - “create pdf file”
  - “generate docx file”
  - “make xlsx”
  - “create backup file for settings”
- These must map to **exact intents** (like `create_pdf_file`) under the correct cluster (`file_ops`).
- Do **NOT infer, assume, or be creative**.
- If matched, return a JSON object with:
  - the `cluster` (file_ops),
  - the matched `intent` (e.g., `create_logfile_with_timestamp`),
  - and the original `user_input`.
- If no intent clearly matches, return `unknown` confidently.

---

🎯 Intent List (`file_ops` cluster):

- create_pdf_file  
- create_docx_file  
- create_rtf_file  
- create_xlsx_file  
- create_temp_file  
- create_backup_file  
- create_logfile_with_timestamp  
- create_config_file  
- create_readme_file  
- create_binary_file  

---

🧠 Output Format:

✅ If matched:
{
  "cluster": "file_ops",
  "intent": "<matched_intent_name>",
  "user_input": "<original_user_input>"
}

'''

    # Append user input to the content
    full_prompt = f"{system_prompt}\n\nuser_input: {user_msg}\nRespond with the correct JSON:"

    client = genai.Client(api_key="AIzaSyDXR4RwrPwanJbQjbnXyT-GYGPAmSYNBOg")
    response = client.models.generate_content(
        model="gemini-2.5-flash",
        contents=full_prompt
    )


    response_text = clean_response(response.text)
    save_json_safely(response_text)


